    ...
    <AnimatePresence>
      {!done && (
        <motion.div
          ...
          style={{ background: "radial-gradient(ellipse at center, #04151c 0%, #01060a 60%, #000305 100%)" }}
        >
          <div className="flex flex-col items-center gap-6">
            <div className="relative h-24 w-24">
              <div className="absolute inset-0 animate-spin-slow rounded-full"
                   style={{ background: "conic-gradient(from 0deg, transparent, #63d8e3, transparent 70%)", filter: "blur(1px)" }} />
                ...
                <motion.div animate={{ y: [0, -3, 0] }} transition={{ duration: 1.4, repeat: Infinity, ease: "easeInOut" }}>
                  <svg width="56" height="42" viewBox="0 0 80 36" fill="none">
                    ...
                    <path d="M2 18 Q20 4 50 18 Q20 32 2 18Z M50 18 L72 6 L68 18 L72 30 Z" fill="url(#lf)"
                        style={{ filter: "drop-shadow(0 0 3px #63d8e3)" }} />
                  </svg>
                </motion.div>
              </div>
            </div>
            <div className="font-display text-[10px] uppercase tracking-[0.5em] text-[#8bd8dc]/80">
              Calibrating depth
            </div>
          </div>
        </motion.div>
